...
class PerplexityMCPServer {
	...
	private determineRecoveryLevel(error?: Error): number {
		...
		if (
			...
			this.log(
				"warn",
				"Detached frame or closed session detected, escalating recovery to level 3 (full restart)",
			);
		...
		if (errorMsg.includes("timeout") || errorMsg.includes("navigation")) {
			this.log(
				"warn",
				"Timeout or navigation error detected, setting recovery to level 1 (refresh)",
			);
		...
		if (errorMsg.includes("internal error")) {
			this.log(
				"warn",
				"Internal error page detected, escalating to level 3 (full restart)",
			);
			return 3; // Full restart for internal errors
		}
		this.log(
			"warn",
			`Unknown error type ('${errorMsg.substring(0, 50)}...'), defaulting recovery to level 3`,
		);
			...
			switch (recoveryLevel) {
				case 1: // Page refresh
					...
					if (this.page && !this.page.isClosed()) {
						...
						this.log(
							"warn",
							"Page was null or closed, cannot refresh. Proceeding with recovery.",
						);
					}
					break;
				...
				case 2: // New page (Currently unused due to level escalation, kept for potential future use)
					...
					if (this.browser && this.browser.isConnected()) {
						try {
							...
							return await this.recoveryProcedure(
								new Error("Fallback recovery: new page failed"),
							);
						}
					} else {
						this.log(
						...
						return await this.recoveryProcedure(
							new Error("Fallback recovery: browser disconnected"),
						);
					}
					break;
				...
				default:
					this.log(
						"info",
						"Performing full browser restart (Recovery Level 3)",
					);
					if (this.page) {
						try {
							if (!this.page.isClosed()) await this.page.close();
		...
		for (let i = 0; i < maxRetries; i++) {
			try {
				...
				if (
					this.page &&
					!this.page.isClosed() &&
					!this.page.mainFrame().isDetached()
				) {
					try {
				...
				if (isTimeoutError) {
					...
					console.log(
						`Waiting ${timeoutWaitTime / 1000} seconds after timeout...`,
					);
			...
			for (let i = 0; i < maxAttempts; i++) {
				...
				if (currentLength > 0) {
					...
					if (noChangeCounter >= 10 && currentLength > 200) {
						console.log(
							"Content stopped growing but has sufficient information",
						);
						break;
					}
				}
	...
	private async performSearch(query: string): Promise<string> {
		...
		try {
			...
			return await this.retryOperation(async () => {
				...
				if (!selector) {
					console.error(
						"Search input not found, taking screenshot for debugging",
					);
					if (this.page) {
						await this.page.screenshot({
...
${codeChunks[0]}`;
	...
	private async _fetchSinglePageContent(url: string): Promise<string> {
		...
		const originalUrl = url; // Keep original for potential Gitingest rewrite
		let pageTitle = "";
		...
		try {
			...
			if (parsedUrl.hostname === "github.com") {
				...
				if (pathParts.length === 2) {
					...
					console.log(
						`Detected GitHub repo URL. Rewriting to: ${gitingestUrl}`,
					);
		...
		if (!isGitHubRepo) {
			try {
				...
				console.warn(
					`HEAD request failed for ${url}: ${headError instanceof Error ? headError.message : String(headError)}. Proceeding with Puppeteer.`,
				);
			}
		} else {
		...
		try {
			...
			if (isGitHubRepo) {
				...
				try {
					await this.page.waitForSelector(".result-text", {
			...
			if (
				...
				console.log(
					`Readability extracted content (${article.textContent.length} chars)`,
				);
				return JSON.stringify(
					{
						status: "Success",
			...
			throw new Error(
				"Readability and fallback selectors failed to extract meaningful content.",
			);
		} catch (error) {
	...
	private async _recursiveFetch(
		...
		try {
			...
			if (currentDepth === 1) {
				...
				if (!this.page || this.page.isClosed())
					throw new Error("Main page instance is not valid for initial fetch.");
				...
				if (pageResult.textContent && pageResult.textContent.length > 20000) {
					pageResult.textContent =
						pageResult.textContent.substring(0, 20000) + "... (truncated)";
				}
				linksToExplore = await this._extractSameDomainLinks(
					this.page,
					startUrl,
				);
			} else {
			...
			console.error(
				`[Depth ${currentDepth}] Error fetching ${startUrl}:`,
				error,
			);
	...
	// --- Main Tool Handler ---
	private async handleExtractUrlContent(args: {
		url: string;
		depth?: number;
	}): Promise<string> {
		...
		if (validatedDepth === 1) {
			...
			console.log(
				`Starting recursive fetch for ${url} up to depth ${validatedDepth}`,
			);
			const visitedUrls = new Set<string>();
			const results: PageContentResult[] = [];
			const startTime = Date.now();
			const globalTimeoutDuration =
				CONFIG.PAGE_TIMEOUT - CONFIG.MCP_TIMEOUT_BUFFER - 5000; // 5s buffer
			let globalTimeoutHandle: NodeJS.Timeout | null = null;
			const globalTimeoutSignal = { timedOut: false };

			const timeoutPromise = new Promise<never>((_, reject) => {
				// Changed to Promise<never>
				globalTimeoutHandle = setTimeout(() => {
					console.warn(
						`Global recursive fetch timeout (${globalTimeoutDuration}ms) reached for ${url}`,
					);
					globalTimeoutSignal.timedOut = true;
					reject(
						new Error(
							`Recursive fetch timed out after ${globalTimeoutDuration}ms`,
						),
					);
			...
			try {
				...
				if (!this.page || this.page.isClosed()) {
					console.log(
						"Page not available for initial recursive fetch, initializing...",
					);
					await this.initializeBrowser(); // Re-initialize if needed
					if (!this.page)
						throw new Error("Failed to initialize page for recursive fetch.");
				}

				const fetchPromise = this._recursiveFetch(
					url,
					validatedDepth,
					1,
					visitedUrls,
				...
				if (globalTimeoutHandle) clearTimeout(globalTimeoutHandle);
				console.error(
					`Recursive fetch failed catastrophically for ${url}:`,
					error,
				);
				const errorMessage =
					...
					inputSchema: {
						...
						properties: {
							message: {
								type: "string",
								description:
									"The message to send to Perplexity AI for web search",
								examples: [
							...
							content: {
								...
								items: {
									...
									properties: {
										url: {
											type: "string",
											description: "URL of the explored page.",
										},
										title: {
											type: "string",
					...
					outputSchema: {
						...
						properties: {
							response: {
								type: "string",
								description:
									"The search result text provided by Perplexity AI.",
							},
						},
					},
					examples: [
